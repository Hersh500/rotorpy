...
from rotorpy.trajectories.minsnap import BatchedMinSnap, MinSnap
...
def minsnap_randomization_fn(batched_minsnap_traj, idx):
    # Generate a new minsnap function. 
    wps = sample_waypoints(3, world, world_buffer=0.0, check_collision=False, min_distance=0.25, max_distance=3, max_attempts=1000, start_waypoint=np.array([0, 0, 0]), end_waypoint=None)
    yaw_angles = np.zeros((3,))
    vavgs = np.random.uniform(low=0.25, high=1.0)

    new_minsnap_traj = MinSnap(wps, yaw_angles, v_avg=vavgs, verbose=False)

    # Assign appropriate values. 
    batched_minsnap_traj.delta_ts[idx] = new_minsnap_traj.delta_t 
    batched_minsnap_traj.t_keyframes[idx] = new_minsnap_traj.t_keyframes
    batched_minsnap_traj.t_keyframes_summed[idx] = new_minsnap_traj.t_keyframes
    batched_minsnap_traj.t_keyframes_summed[idx, :-1] += new_minsnap_traj.delta_t
    batched_minsnap_traj.xs[idx] = torch.from_numpy(new_minsnap_traj.x_poly).to(device, non_blocking=True)
    batched_minsnap_traj.x_dots[idx] = torch.from_numpy(new_minsnap_traj.x_dot_poly).to(device, non_blocking=True)
    batched_minsnap_traj.x_ddots[idx] = torch.from_numpy(new_minsnap_traj.x_ddot_poly).to(device, non_blocking=True)
    batched_minsnap_traj.yaws[idx] = torch.from_numpy(new_minsnap_traj.yaw_poly).to(device, non_blocking=True)
    batched_minsnap_traj.yaw_dots[idx] = torch.from_numpy(new_minsnap_traj.yaw_dot_poly).to(device, non_blocking=True)

randomizations = {}
randomizations["mass"] = [0.027, 0.041]
randomizations["k_eta"] = [3.0e-8, 4.0e-8]
randomizations["tau_m"] = [0.004, 0.010]
...
reset_options["vel_bound"] = 0.75
reset_options["traj_randomization_fn"] = minsnap_randomization_fn 
...
for _ in range(num_envs):
    wps = sample_waypoints(3, world, world_buffer=0.0, check_collision=False, min_distance=0.25, max_distance=3, max_attempts=1000, start_waypoint=np.array([0, 0, 0]), end_waypoint=None)
    yaw_angles = np.zeros((3,))
    vavgs = np.random.uniform(low=0.25, high=3.0)
    minsnap_trajs.append(MinSnap(wps, yaw_angles, v_avg=vavgs, verbose=False))

trajectory = BatchedMinSnap(minsnap_trajs, device=device)
