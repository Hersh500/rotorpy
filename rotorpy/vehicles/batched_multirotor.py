...
class BatchedMultirotor(object):
    ...
    def __init__(self, quad_params,
        ...
        self.inertia = torch.tensor([[self.Ixx, self.Ixy, self.Ixz],
                                 ...
                                 [self.Ixz, self.Iyz, self.Izz]], device=device).unsqueeze(0).float()  # will be useful to add a batch dim
        ...
        self.f_to_TM = torch.from_numpy(np.vstack((np.ones((1,self.num_rotors)),
                                                   ...
                                                   (k * self.rotor_dir).reshape(1,-1)))).float().to(device)
        self.rotor_dir = self.rotor_dir.float().to(device)
    ...
    def extract_geometry(self):
        ...
        self.rotor_geometry = torch.from_numpy(self.rotor_geometry).float().unsqueeze(0).to(self.device)  # naive, we could instead build the torch tensor directly.
        return
    ...
    def step(self, state, control, t_step, idxs=None, debug=False):
        ...
        s = BatchedMultirotor._pack_state(state, self.num_drones, self.device).float()

        # Option 1 - RK45 integration
        # sol = scipy.integrate.solve_ivp(s_dot_fn, (0, t_step), s, first_step=t_step)
        sol = odeint(s_dot_fn, s[idxs], t=torch.tensor([0.0, t_step], dtype=torch.float32, device=self.device), method='dopri5', options={"dtype":torch.float32})
        # s = sol['y'][:,-1]
        s = sol[-1,:]
    ...
    def _pack_state(cls, state, num_drones, device):
        ...
        """
        s = torch.zeros(num_drones, 20, device=device).float()   # FIXME: this shouldn't be hardcoded. Should vary with the number of rotors.
    ...
    def _unpack_state(cls, s, idxs, num_drones):
        ...
        state = {'x': torch.zeros(num_drones, 3, device=device).float(),
                 'v': torch.zeros(num_drones, 3, device=device).float(),
                 'q': torch.zeros(num_drones, 4, device=device).float(),
                 'w': torch.zeros(num_drones, 3, device=device).float(),
                 'wind': torch.zeros(num_drones, 3, device=device).float(),
                 'rotor_speeds': torch.zeros(num_drones, 4, device=device).float()}
