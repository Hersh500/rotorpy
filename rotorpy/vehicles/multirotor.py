...
# AI-generated refactor of BatchedMultirotor to make it JIT-friendly.
class JITMultirotor(object):
    """
    JIT-friendly Multirotor forward dynamics model, implemented in PyTorch.
    This class refactors BatchedMultirotor to take parameters directly as method arguments
    instead of using a BatchedMultirotorParams object, making it more suitable for JIT compilation.
    ...
    def __init__(self, num_drones, initial_states, device, control_abstraction='cmd_motor_speeds', aero=True, integrator='dopri5'):
        """
        Initialize JITMultirotor with minimal state.
        """
        assert initial_states['x'].device == device, "Initial states must already be on the specified device."
        assert initial_states['x'].shape[0] == num_drones
        self.num_drones = num_drones
    ...
    def statedot(self, state, control, t_step, idxs, rotor_speed_min, rotor_speed_max, mass, tau_m, weight, inv_inertia, inertia, rotor_geometry, k_eta, drag_matrix, rotor_drag_matrix, k_flap, k_h, rotor_geometry_hat_maps, rotor_dir, k_m, num_rotors, k_w, g, kp_att, kd_att, TM_to_f):
        """
        Integrate dynamics forward from state given constant cmd_rotor_speeds for time t_step.
        Returns:
            vdot: torch.tensor of shape (num_drones, 3), with zeros for drones not in idxs
        ...
        cmd_rotor_speeds = self.get_cmd_motor_speeds(state, control, idxs, k_eta, k_w, inertia, mass, kp_att, kd_att, TM_to_f, g)

        # The true motor speeds can not fall below min and max speeds.
        cmd_rotor_speeds = torch.clip(cmd_rotor_speeds, rotor_speed_min[idxs], rotor_speed_max[idxs])

        # Form autonomous ODE for constant inputs and integrate one time step.
        def s_dot_fn(t, s):
            return self._s_dot_fn(t, s, cmd_rotor_speeds, idxs, tau_m, weight, mass, inv_inertia, inertia, rotor_geometry, k_eta, drag_matrix, rotor_drag_matrix, k_flap, k_h, rotor_geometry_hat_maps, rotor_dir, k_m, num_rotors)
    ...
    def step(self, state, control, t_step, idxs, rotor_speed_min, rotor_speed_max, num_rotors, motor_noise, mass, tau_m, weight, inv_inertia, inertia, rotor_geometry, k_eta, drag_matrix, rotor_drag_matrix, k_flap, k_h, rotor_geometry_hat_maps, rotor_dir, k_m, k_w, kp_att, kd_att, TM_to_f, g):
        ...
        state = JITMultirotor._unpack_state(s, idxs, self.num_drones)

        # Re-normalize unit quaternion.
        state['q'][idxs] = state['q'][idxs] / torch.norm(state['q'][idxs], dim=-1).unsqueeze(-1)

        # Add noise to the motor speed measurement
        # state['rotor_speeds'][idxs] += torch.normal(mean=torch.zeros(num_rotors[0], device=self.device),
        #                                             std=torch.ones(len(idxs), num_rotors[0], device=self.device) * torch.abs(motor_noise[idxs]))
        state['rotor_speeds'][idxs] = torch.clip(state['rotor_speeds'][idxs], rotor_speed_min[idxs], rotor_speed_max[idxs])

        return state

    def _s_dot_fn(self, t, s, cmd_rotor_speeds, idxs, tau_m, weight, mass, inv_inertia, inertia, rotor_geometry, k_eta, drag_matrix, rotor_drag_matrix, k_flap, k_h, rotor_geometry_hat_maps, rotor_dir, k_m, num_rotors):
        """
        Compute derivative of state for quadrotor given fixed control inputs as
        an autonomous ODE.
        """
        ...
        # Rotor speed derivative
        rotor_accel = (1 / tau_m[idxs]) * (cmd_rotor_speeds - rotor_speeds)
        ...
        # Compute total wrench in the body frame based on the current rotor speeds and their location w.r.t. CoM
        (FtotB, MtotB) = self.compute_body_wrench(state['w'][idxs], rotor_speeds, body_airspeed_vector, idxs, rotor_geometry, k_eta, drag_matrix, rotor_drag_matrix, k_flap, k_h, rotor_geometry_hat_maps, rotor_dir, k_m, num_rotors)

        # Rotate the force from the body frame to the inertial frame
        Ftot = R @ FtotB.unsqueeze(-1)

        # Velocity derivative.
        v_dot = (weight[idxs] + Ftot.squeeze(-1)) / mass[idxs]

        # Angular velocity derivative.
        w = state['w'][idxs].double()
        w_hat = JITMultirotor.hat_map(w).permute(2, 0, 1)
        w_dot = inv_inertia[idxs] @ (MtotB - (w_hat.double() @ (inertia[idxs] @ w.unsqueeze(-1))).squeeze(-1)).unsqueeze(-1)

        # NOTE: the wind dynamics are currently handled in the wind_profile object.
        wind_dot = torch.zeros((len(idxs), 3), device=self.device)

        # Pack into vector of derivatives.
        s_dot = torch.zeros((len(idxs), 16 + num_rotors[0],), device=self.device)
        s_dot[:, 0:3] = x_dot
        s_dot[:, 3:6] = v_dot
        s_dot[:, 6:10] = q_dot
        s_dot[:, 10:13] = w_dot.squeeze(-1)
    ...
    def compute_body_wrench(self, body_rates, rotor_speeds, body_airspeed_vector, idxs, rotor_geometry, k_eta, drag_matrix, rotor_drag_matrix, k_flap, k_h, rotor_geometry_hat_maps, rotor_dir, k_m, num_rotors):
        """
        Computes the wrench acting on the rigid body based on the rotor speeds for thrust and airspeed
        for aerodynamic forces.
        """
        ...
        # Get the local airspeeds for each rotor
        local_airspeeds = body_airspeed_vector.unsqueeze(-1) + (
            JITMultirotor.hat_map(body_rates).permute(2, 0, 1)) @ (rotor_geometry[idxs].transpose(1, 2))

        # Compute the thrust of each rotor, assuming that the rotors all point in the body z direction!
        T = torch.zeros(num_drones, 3, 4, device=self.device)
        T[..., -1, :] = k_eta[idxs] * rotor_speeds ** 2

        # Add in aero wrenches (if applicable)
        if self.aero:
            # Parasitic drag force acting at the CoM
            tmp = drag_matrix[idxs] @ (body_airspeed_vector).unsqueeze(-1)
            D = -JITMultirotor._norm(body_airspeed_vector).unsqueeze(-1) * tmp.squeeze()
            # Rotor drag (aka H force) acting at each propeller hub.
            tmp = rotor_drag_matrix[idxs] @ local_airspeeds.double()
            H = -rotor_speeds.unsqueeze(1) * tmp
            # Pitching flapping moment acting at each propeller hub.
            M_flap = JITMultirotor.hat_map(local_airspeeds.transpose(1, 2).reshape(num_drones * 4, 3))
            M_flap = M_flap.permute(2, 0, 1).reshape(num_drones, 4, 3, 3).double()
            M_flap = M_flap @ torch.tensor([0, 0, 1.0], device=self.device).double()
            M_flap = (-k_flap[idxs] * rotor_speeds).unsqueeze(1) * M_flap.transpose(-1, -2)
        ...
        M_yaw = torch.zeros(num_drones, 3, 4, device=self.device)
        M_yaw[..., -1, :] = rotor_dir[idxs] * k_m[idxs] * rotor_speeds ** 2
    ...
    def get_cmd_motor_speeds(self, state, control, idxs, k_eta, k_w, inertia, mass, kp_att, kd_att, TM_to_f, g):
        """
        Computes the commanded motor speeds depending on the control abstraction.
        """
        ...
        if self.control_abstraction == 'cmd_motor_speeds':
            ...
            cmd_motor_speeds = control["cmd_motor_thrusts"][idxs] / k_eta[idxs]
            return torch.sign(cmd_motor_speeds) * torch.sqrt(torch.abs(cmd_motor_speeds))
        elif self.control_abstraction == "cmd_ctbm":
            cmd_thrust = control['cmd_thrust'][idxs]
            cmd_moment = control['cmd_moment'][idxs]
            ...
            # Computed commanded moment based on the attitude error and body rate error
            wdot_cmd = -k_w[idxs] * w_err
            cmd_moment = inertia[idxs] @ wdot_cmd.unsqueeze(-1)
        elif self.control_abstraction == "cmd_vel":
            # The controller commands a velocity vector.
            v_err = state['v'][idxs] - control['cmd_v'][idxs]

            # Get desired acceleration based on P control of velocity error.
            a_cmd = -k_w[idxs] * v_err

            # Get desired force from this acceleration.
            F_des = mass[idxs] * (a_cmd + torch.tensor([0, 0, g[0]], device=self.device))
            ...
            Iw = inertia[idxs] @ state['w'][idxs].unsqueeze(-1).double()
            tmp = -kp_att[idxs] * att_err - kd_att[idxs] * state['w'][idxs]
            cmd_moment = (inertia[idxs] @ tmp.unsqueeze(-1)).squeeze(-1) + torch.cross(state['w'][idxs], Iw.squeeze(-1), dim=-1)
        elif self.control_abstraction == "cmd_ctatt":
            cmd_thrust = control["cmd_thrust"][idxs]
            R = roma.unitquat_to_rotmat(state['q'][idxs]).double()
            R_des = roma.unitquat_to_rotmat(control["cmd_q"][idxs]).double()
            ...
            F_des = control['cmd_acc'][idxs] * mass[idxs]
            R = roma.unitquat_to_rotmat(state['q'][idxs]).double()
            b3 = R @ torch.tensor([0.0, 0.0, 1.0], device=self.device).double()
            cmd_thrust = torch.sum(F_des * b3, dim=-1).double().unsqueeze(-1)
        ...
        TM = torch.cat([cmd_thrust, cmd_moment.squeeze(-1)], dim=-1)
        cmd_rotor_thrusts = (TM_to_f[idxs] @ TM.unsqueeze(1).transpose(-1, -2)).squeeze(-1)
        cmd_motor_speeds = cmd_rotor_thrusts / k_eta[idxs]
        cmd_motor_speeds = torch.sign(cmd_motor_speeds) * torch.sqrt(torch.abs(cmd_motor_speeds))
        return cmd_motor_speeds
    ...
    def _pack_state(cls, state, num_drones, device):
        ...
        Convert a state dict to Quadrotor's private internal vector representation.
        """
        s = torch.zeros(num_drones, 20, device=device).double()
        s[..., 0:3] = state['x']  # inertial position
        s[..., 3:6] = state['v']  # inertial velocity
        s[..., 6:10] = state['q']  # orientation
        s[..., 10:13] = state['w']  # body rates
