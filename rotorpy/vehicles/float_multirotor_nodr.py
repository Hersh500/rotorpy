...
class BatchedMultirotorFloatParamsNoDR:
    """ 
    A container class for various multirotor params. 
    Parameters:
        multirotor_params: dictionary containing the parameters for each drone (see vehicles/crazyflie_params.py for example of such a dictionary) - notably, NOT A LIST.
        device: the device to use for the simulation (e.g. torch.device('cuda')).
    """
    def __init__(self, multirotor_params, device):
        qp = multirotor_params
        self.device = device
        self.mass = multirotor_params['mass']

        self.num_rotors = multirotor_params["num_rotors"]

        self.rotor_pos = multirotor_params["rotor_pos"]

        self.rotor_dir_np       = multirotor_params['rotor_directions']

        self.extract_geometry()

        # Rotor parameters
        self.rotor_speed_min = multirotor_params['rotor_speed_min']
        self.rotor_speed_max = multirotor_params['rotor_speed_max']

        self.k_eta = multirotor_params['k_eta']
        self.k_m = multirotor_params['k_m']
        self.k_flap = multirotor_params['k_flap']
        self.k_h = multirotor_params['k_h']

        # Motor parameters
        self.tau_m           = multirotor_params['tau_m']
        self.motor_noise     = multirotor_params['motor_noise_std']

        # Additional constants.
        self.inertia = torch.from_numpy(np.array([[qp["Ixx"], qp["Ixy"], qp["Ixz"]],
                                                   [qp["Ixy"], qp["Iyy"], qp["Iyz"]],
                                                   [qp["Ixz"], qp["Iyz"], qp["Izz"]]])).float().to(device)
        self.rotor_drag_matrix = torch.tensor([[qp["k_d"],   0,                 0],
                                                [0,          qp["k_d"],          0],
                                                [0,          0,          qp["k_z"]]]).float().to(device)

        self.drag_matrix = torch.tensor([[qp["c_Dx"],   0,                 0],
                                          [0,          qp["c_Dy"],          0],
                                          [0,          0,          qp["c_Dz"]]]).float().to(device)

        # keep this the same across drones.
        self.g = 9.81 # m/s^2

        self.inv_inertia = torch.linalg.inv(self.inertia).float()
        self.weight = torch.zeros(3, device=device).float()
        ...
        self.f_to_TM = torch.from_numpy(np.vstack((np.ones((1,self.num_rotors)),
                                            ...
                                            (k * self.rotor_dir_np).reshape(1,-1)))).to(device)
        ...
        self.k_w = multirotor_params.get('k_w', 1)
        self.k_v = multirotor_params.get('k_v', 10)
        self.kp_att = multirotor_params.get('kp_att', 3000.0)
        self.kd_att = multirotor_params.get('kd_att', 360.0)
    ...
    def extract_geometry(self):
        ...
        geom_hat_map = BatchedMultirotorFloatNoDR.hat_map(torch.from_numpy(self.rotor_geometry.squeeze())).numpy()
        self.rotor_geometry = torch.from_numpy(self.rotor_geometry).unsqueeze(0).float().to(self.device)  # naive, we could instead build the torch tensor directly.
        self.rotor_geometry_hat_maps = torch.from_numpy(np.array(geom_hat_map)).float().to(self.device)
        return


class BatchedMultirotorFloatNoDR(object):
    """
    Batched Multirotor forward dynamics model, implemented in PyTorch. Uses Float tensors
    ...
    def statedot(self, state, control, t_step, idxs):
        ...
        cmd_rotor_speeds = torch.clip(cmd_rotor_speeds, self.params.rotor_speed_min,
        ...
        s = BatchedMultirotorFloatNoDR._pack_state(state, self.num_drones, self.device)
    ...
    def step(self, state, control, t_step, idxs=None):
        ...
        state = BatchedMultirotorFloatNoDR._unpack_state(s, idxs, self.num_drones)
        ...
        state['rotor_speeds'][idxs] += torch.normal(mean=torch.zeros(self.params.num_rotors, device=self.device),
                                                    std=torch.ones(len(idxs), self.params.num_rotors,
                                                                   device=self.device) * np.abs(
                                                        self.params.motor_noise))
        state['rotor_speeds'][idxs] = torch.clip(state['rotor_speeds'][idxs], self.params.rotor_speed_min,
                                                 self.params.rotor_speed_max)
    ...
    def _s_dot_fn(self, t, s, cmd_rotor_speeds, idxs):
        ...
        rotor_accel = (1 / self.params.tau_m) * (cmd_rotor_speeds - rotor_speeds)
        ...
        # Velocity derivative.
        v_dot = (self.params.weight + Ftot.squeeze(-1)) / self.params.mass

        # Angular velocity derivative.
        w = state['w'][idxs].float()
        w_hat = BatchedMultirotorFloatNoDR.hat_map(w).permute(2, 0, 1)
        w_dot = self.params.inv_inertia @ (
                    MtotB - (w_hat.float() @ (self.params.inertia @ w.unsqueeze(-1))).squeeze(-1)).unsqueeze(-1)
    ...
    def compute_body_wrench(self, body_rates, rotor_speeds, body_airspeed_vector, idxs):
        ...
        local_airspeeds = body_airspeed_vector.unsqueeze(-1) + (
            BatchedMultirotorFloatNoDR.hat_map(body_rates).permute(2, 0, 1)) @ (self.params.rotor_geometry.transpose(1, 2))
        ...
        T[..., -1, :] = self.params.k_eta * rotor_speeds ** 2

        # Add in aero wrenches (if applicable)
        if self.aero:
            # Parasitic drag force acting at the CoM
            tmp = self.params.drag_matrix @ (body_airspeed_vector).unsqueeze(-1)
            D = -BatchedMultirotorFloatNoDR._norm(body_airspeed_vector).unsqueeze(-1) * tmp.squeeze()
            # Rotor drag (aka H force) acting at each propeller hub.
            tmp = self.params.rotor_drag_matrix @ local_airspeeds.float()
            H = -rotor_speeds.unsqueeze(1) * tmp
            # Pitching flapping moment acting at each propeller hub.
            M_flap = BatchedMultirotorFloatNoDR.hat_map(local_airspeeds.transpose(1, 2).reshape(num_drones * 4, 3))
            M_flap = M_flap.permute(2, 0, 1).reshape(num_drones, 4, 3, 3).float()
            M_flap = M_flap @ torch.tensor([0, 0, 1.0], device=self.device).float()
            M_flap = (-self.params.k_flap * rotor_speeds).unsqueeze(1) * M_flap.transpose(-1, -2)

            lift = torch.zeros(num_drones, 3, 1, device=self.device).float()
            lift[:, 2, :] = self.params.k_h
        ...
        M_force = -torch.einsum('bijk, bik->bj', self.params.rotor_geometry_hat_maps.unsqueeze(0), T + H)
        M_yaw = torch.zeros(num_drones, 3, 4, device=self.device)
        M_yaw[..., -1, :] = self.params.rotor_dir * self.params.k_m * rotor_speeds ** 2
    ...
    def get_cmd_motor_speeds(self, state, control, idxs):
        ...
        if self.control_abstraction == 'cmd_motor_speeds':
            ...
            cmd_motor_speeds = control["cmd_motor_thrusts"][idxs].float() / self.params.k_eta
            ...
            wdot_cmd = -self.params.k_w * w_err
            cmd_moment = self.params.inertia @ wdot_cmd.unsqueeze(-1)
            ...
            a_cmd = -self.params.k_v * v_err

            # Get desired force from this acceleration.
            F_des = self.params.mass * (a_cmd + np.array([0, 0, self.params.g]))
            ...
            Iw = self.params.inertia @ state['w'][idxs].unsqueeze(-1).float()
            tmp = -self.params.kp_att * att_err - self.params.kd_att * state['w']
            cmd_moment = (self.params.inertia @ tmp.unsqueeze(-1)).squeeze(-1) + torch.cross(state['w'][idxs],
                                                                                                   Iw.squeeze(-1),
                                                                                                   dim=-1)
            ...
            att_err = torch.stack([-S_err[:, 1, 2], S_err[:, 0, 2], -S_err[:, 0, 1]], dim=-1)
            Iw = self.params.inertia @ state['w'][idxs].unsqueeze(-1).float()
            tmp = -self.params.kp_att * att_err - self.params.kd_att * state['w'][idxs]
            cmd_moment = (self.params.inertia @ tmp.unsqueeze(-1)).squeeze(-1) + torch.cross(state['w'][idxs],
                                                                                                   Iw.squeeze(-1),
                                                                                                   dim=-1)
        elif self.control_abstraction == "cmd_acc":
            F_des = control['cmd_acc'][idxs].float() * self.params.mass
            R = roma.unitquat_to_rotmat(state['q'][idxs]).float()
            b3 = R @ torch.tensor([0.0, 0.0, 1.0], device=self.device).float()
            cmd_thrust = torch.sum(F_des * b3, dim=-1).float().unsqueeze(-1)
        ...
        cmd_rotor_thrusts = (self.params.TM_to_f @ TM.unsqueeze(1).transpose(-1, -2)).squeeze(-1)
        cmd_motor_speeds = cmd_rotor_thrusts / self.params.k_eta
